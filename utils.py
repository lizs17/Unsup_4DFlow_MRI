...
def removeStringPartBySplit(inStr, spliter, rmvIdxs):
    spltStr = inStr.split(spliter)
    # substitute rmv element to be ''
    for ii in rmvIdxs:
        spltStr[ii] = ''
    # concate
    outStr = ''
    for ii in range(len(spltStr)):
        if (ii > 0) and (not (ii in rmvIdxs)):
            outStr += spliter
        outStr += spltStr[ii]
    return outStr


def removeStringPostfix(inStr, spliter):
    N = len(inStr.split(spliter))
    assert N > 1
    outStr = removeStringPartBySplit(inStr, spliter, [N-1])
    return outStr


def removeStringPrefix(inStr, spliter):
    N = len(inStr.split(spliter))
    assert N > 1
    outStr = removeStringPartBySplit(inStr, spliter, [0])
    return outStr


def removeFilePostfix(filename):
    out = removeStringPostfix(inStr=filename, spliter='.')
    return out
...
#region MRI mask generation for numpy

def normal_pdf(length, sensitivity):
    return np.exp(-sensitivity * (np.arange(length) - length / 2)**2)

def genMask_Cartesian_2D(shape, R, ACS, centred, Nlines=None):
    """
    Generate random 2D Cartesian undersampling mask
    Sampling density estimated from implementation of k-t FOCUSS
    Input:
        shape: [..., Ny, Nx]
        R: acceleration factor (doesn't have to be integer)
        ACS: fully-sampled lines in the center region
        centred: whether the mask is centred
        Nlines: number of acquired lines
    Output:
        [..., Ny, Nx]
    """
    N, Ny, Nx = int(np.prod(shape[:-2])), shape[-2], shape[-1]
    # get probability density function
    pdf_y = normal_pdf(Ny, 0.5 / (Ny / 10.) ** 2)
    # 'Nlines' will overwrite 'R'
    if Nlines is None:
        Nlines = int(Ny / R)
    else:
        R = Ny / Nlines
    lmda = Ny / (2. * R)
    pdf_y += lmda * 1./Ny  # add uniform distribution
    # throw-out the ACS lines
    if ACS > 0:
        ACS_start = Ny//2 - ACS//2
        pdf_y[ACS_start:(ACS_start+ACS)] = 0
        pdf_y /= np.sum(pdf_y)
        # check whether ACS is proper
        if Nlines < (2*ACS):
            print('!!!!!!!!!!!! Nlines:ACS is {0}:{1}, the outer k-space may be undersampled too much'.format(Nlines, ACS))
        if Nlines < ACS:
            raise ValueError('Nlines={0} is less than ACS={1}'.format(Nlines, ACS))
        Nlines -= ACS
    # generate mask
    mask = np.zeros((N, Ny))
    for i in range(N):
        # for each mask, the lines are selected according to the pdf
        idx = np.random.choice(Ny, Nlines, False, pdf_y)
        mask[i, idx] = 1
    # set ACS region to 1
    if ACS > 0:
        ACS_start = Ny // 2 - ACS // 2
        mask[:, ACS_start:(ACS_start+ACS)] = 1
    # Nx is fully-sampled
    mask = np.reshape(mask,(N,Ny,1))
    mask = np.repeat(mask, Nx, axis=2)
    # output shape
...
def remove_freq_ovs(x, dim):
    Nread = x.shape[dim]
    assert Nread % 2 == 0
    Nx = Nread // 2
    # ------
    out = ifft1c_norm_np(x=x, dim=dim)
    out = np.swapaxes(out, axis1=0, axis2=dim)
    out = out[(Nx//2):(Nx//2+Nx), ...]
    out = np.swapaxes(out, axis1=0, axis2=dim)
    out = fft1c_norm_np(x=out, dim=dim)
    return out
...
def read_txt_intoLines(txt_path):
    output = []
...
def write_txt_fromLines(txt_path, lineList):
    with open(txt_path, 'a') as f:
        for ii in lineList:
            f.write(str(lineList[ii]))
